# File: python/app/server/adventure_private.py ✅
# Version: 3.0
...
def handle_private_request(handler):
    ...
    show_private = (reqUser != "" and reqUser.lower() == "clan-wallace")
    ...
    ALsplit = (
        ...
        f"<p>"
        f"<span style='background:#060;color:#fff;border-radius:5px;padding:3px 5px;'>{gc_note}</span> "
    )
    if duration:
    ...
    for stage in stages:
        ...
        if isinstance(slat, float):
            slat = f"{slat:.12f}".rstrip("0").rstrip(".")
        if isinstance(slon, float):
            slon = f"{slon:.12f}".rstrip("0").rstrip(".")
        ...
        if isinstance(geofence, float):
            geofence = int(geofence)
        ...
        if isinstance(journal.get("image"), dict):
            awardImage = journal["image"].get("url", "")
        ...
        LBsplitScreen = (
            ...
            f"</p>"
            f"<div>{LBdescriptionHtml}</div>"
            "</div>"
            f"<p style='text-align: center;'>"
            f"<img src='{LBimage}' style='max-width:80%; max-height:80%; border:6px solid white; border-radius:20px; box-shadow:5px 5px 7px rgba(0,0,0,0.5);'/>"
            "</p>"
            f"{ALshort}"
            f"{ALsplit}"
            "</div>"
        ...
        if show_private:
            stage_row.append(answer)
            stage_row.append(awardImage)
            stage_row.append(message_plain)
...
# ---------------- FINAL USER NOTE AND CP1252 OUTPUT ----------------
    ...
    try:
        ...
        # Fallback to UTF-8 if CP1252 fails, though GSAK prefers 1252
